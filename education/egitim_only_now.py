...
def train_loop(model, train_loader, val_loader, device, epochs, model_name):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1} -> Loss: {epoch_loss:.4f} | Val Acc: {val_acc:.4f} | LR: {optimizer.param_groups[0]['lr']}")

        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), save_path)
            print(f"--> [KAYDEDİLDİ] Yeni en iyi: {best_acc:.4f}")
            patience_counter = 0
        else:
            patience_counter += 1
            if patience_counter >= EARLY_STOP_PATIENCE:
                print(f"!!! EARLY STOPPING: {model_name} durduruldu.")
                break
...
def main():
    ...
    # --- 1. EFFICIENTNET-B0 ---
    print("\n>>> 1/2: EfficientNet-B0 Başlatılıyor...")
    ...
    results['EfficientNet_B0'] = train_loop(model_eff, train_loader, val_loader, device, EPOCHS, "EfficientNet_B0")

    del model_eff
    torch.cuda.empty_cache()

    # --- 2. COATNET ---
    print("\n>>> 2/2: CoAtNet Başlatılıyor...")
    try:
        model_coat = timm.create_model('coatnet_0_rw_224', pretrained=True, num_classes=num_classes).to(device)
        results['CoAtNet'] = train_loop(model_coat, train_loader, val_loader, device, EPOCHS, "CoAtNet")
        del model_coat
        torch.cuda.empty_cache()
    ...
    # --- ÖZET RAPOR ---
    print("\n" + "="*30)
    print("EĞİTİM TAMAMLANDI ÖZETİ")
    for m_name, acc in results.items():
        print(f"{m_name}: En İyi Başarı = %{acc*100:.2f}")
    print("="*30)
