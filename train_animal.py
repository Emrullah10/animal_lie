...
# =========================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Hayvan Hastalıkları Sınıflandırma - Train (Hayvan Bazlı)")
    parser.add_argument("--data_dir",          type=str,   default="./split")
    ...
    parser.add_argument(
        "--animals", nargs="+",
        default=["cat", "dog", "cattle", "pig", "poultry"],
        help="Eğitilecek hayvan sınıfları: cat dog cattle pig poultry"
    )
    parser.add_argument(
...
# =========================================================
def get_animal_for_class(class_name):
    cls_lower = class_name.lower()
    if 'cattle' in cls_lower or 'cow' in cls_lower:
        return 'cattle'
    elif 'cat' in cls_lower:
        return 'cat'
    elif 'dog' in cls_lower:
        return 'dog'
    elif 'pig' in cls_lower or 'swine' in cls_lower:
        return 'pig'
    elif 'poultry' in cls_lower or 'chicken' in cls_lower:
        return 'poultry'
    return None

class AnimalFilteredImageFolder(datasets.ImageFolder):
    def __init__(self, root, animal, transform=None, *args, **kwargs):
        self.animal = animal
        super().__init__(root, transform=transform, *args, **kwargs)

    def find_classes(self, directory):
        all_classes, all_class_to_idx = super().find_classes(directory)
        filtered_classes = [c for c in all_classes if get_animal_for_class(c) == self.animal]
        filtered_class_to_idx = {cls_name: i for i, cls_name in enumerate(sorted(filtered_classes))}
        return sorted(filtered_classes), filtered_class_to_idx
...
def get_model_save_paths(base_name, resume=False):
    # base_name formatı: "animal_ModelName" (örn: "cat_ConvNeXtV2-Base")
    # Hayvan ismini ayıklayıp checkpoints/<run_dir>/<hayvan_ismi>/ altına kaydeder
    if "_" in base_name:
        animal, model_part = base_name.split("_", 1)
    else:
        animal, model_part = "general", base_name

    save_dir = os.path.join("checkpoints", RUN_DIR or "run_animal", animal)
    ...
    if resume:
        ...
        found_counter = 1
        while True:
            best_path = os.path.join(save_dir, f"{model_part}_best_v{counter}.pth")
            latest_path = os.path.join(save_dir, f"{model_part}_latest_v{counter}.pth")
            if os.path.exists(best_path) or os.path.exists(latest_path):
                found_counter = counter
                counter += 1
            else:
                break
        best_path = os.path.join(save_dir, f"{model_part}_best_v{found_counter}.pth")
        latest_path = os.path.join(save_dir, f"{model_part}_latest_v{found_counter}.pth")
...
def main():
    ...
    if args.resume:
        if args.resume.lower() == 'latest':
            ...
            if os.path.exists(checkpoints_dir):
                subdirs = [d for d in os.listdir(checkpoints_dir) if os.path.isdir(os.path.join(checkpoints_dir, d)) and d.startswith("run_animal_")]
                if subdirs:
                    subdirs.sort()
                    ...
                    print("   [RESUME UYARI] 'run_animal_' ile başlayan hiçbir klasör bulunamadı. Yeni eğitim başlatılıyor.")
                    run_time = datetime.datetime.now().strftime("%Y%m%d_%H%M")
                    RUN_DIR = f"run_animal_{run_time}"
            else:
    ...
    print(f"Seçilen Hayvan Sınıfları: {args.animals}\n")
    ...
    animals_to_train = [a.lower() for a in args.animals]

    # Klasör yolları belirleme (val/valid fallback dahil)
    train_dir = os.path.join(args.data_dir, 'train')
    val_dir = os.path.join(args.data_dir, 'val')
    if not os.path.exists(val_dir):
        val_dir = os.path.join(args.data_dir, 'valid')

    for animal in animals_to_train:
        print(f"\n{'='*70}")
        print(f"[*]  HAYVAN SINIFI: {animal.upper()} EGITIMI BASLIYOR  [*]")
        print(f"{'='*70}")

        # ---- DATASET ----
        try:
            train_dataset = AnimalFilteredImageFolder(train_dir, animal, transform=train_transforms)
            val_dataset   = AnimalFilteredImageFolder(val_dir,   animal, transform=val_transforms)
        except Exception as e:
            print(f"[HATA] {animal} için dataset yüklenemedi: {e}")
            continue

        num_classes = len(train_dataset.classes)
        if num_classes == 0:
            print(f"[UYARI] {animal} için sınıf bulunamadı, atlanıyor.")
            continue

        print(f"Sınıf Sayısı ({animal}): {num_classes}")
        ...
        for key in selected:
            ...
            # Hayvana özel model adı
            animal_model_name = f"{animal}_{display_name}"
            print(f"\n>>> {animal_model_name} başlatılıyor...")

            try:
                model = build_fn(num_classes).to(device, memory_format=torch.channels_last)
                train_loop(model, key, train_loader, val_loader,
                           device, animal_model_name, class_weights, args)
            except Exception as e:
                print(f"[HATA] {animal_model_name}: {e}")
