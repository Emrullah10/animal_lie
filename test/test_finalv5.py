...
SAVE_DIR_ROOT = "final_analiz_raporu4"
os.makedirs(SAVE_DIR_ROOT, exist_ok=True)

# Gerekli dönüşümler (Değerlendirme sırasında train, valid ve test için aynı dönüşümler kullanılır)
...
# Veri setlerini dinamik olarak yükle (train, valid, test)
...
for split in splits:
    ...
    if os.path.exists(split_dir):
        dataset = datasets.ImageFolder(split_dir, transform=eval_transforms)
        # Değerlendirme yapıldığı için shuffle=False yapılmalıdır
...
def evaluate_model(path, model_type, log_path, model_name, data_loaders):
    ...
    # Eğitim Geçmişi (Sadece bir kere ana klasöre çizilir)
    plot_training_history(log_path, model_save_dir, model_name)

    # Bulunan tüm veri setleri (train, valid, test) üzerinde sırayla analiz yap
    for split_name, loader in data_loaders.items():
        print(f"   ➤ {split_name.upper()} veri seti üzerinden değerlendirme yapılıyor...")

        # Bu veri setine özel alt klasör oluştur
        split_save_dir = os.path.join(model_save_dir, split_name)
        ...
        # Görselleştirme isminde split adını da kullanıyoruz (Örn: ResNeSt_train)
        split_model_name = f"{model_name}_{split_name}"

        # Test Metrikleri (Confusion Matrix, ROC, PR Curves)
        plot_detailed_metrics(all_labels_np, all_preds_np, all_probs_np, class_names, split_save_dir, split_model_name)

        # Raporu Kaydet ve Çubuk Grafiği Çizdir
        ...
        with open(os.path.join(split_save_dir, "report.json"), "w") as f:
            json.dump(report_dict, f, indent=4)

        plot_f1_precision_recall_bars(report_dict, class_names, split_save_dir, split_model_name)

        macro_f1 = report_dict['macro avg']['f1-score']
        print(f"      ✅ {split_name.upper()} bitti -> Macro F1 : {macro_f1:.4f} | Macro AUC: {macro_auc:.4f}")

if __name__ == "__main__":
    # Sadece 34 sınıfa uygun olan modeller test edilecek.
    models_to_test = [
    ...
    for m in models_to_test:
        if os.path.exists(m["path"]):
            evaluate_model(m["path"], m["type"], m["log"], m["name"], loaders)
        else:
